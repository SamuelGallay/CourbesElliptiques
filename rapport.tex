...
\begin{deff}
Pour définir la somme de deux points $P_1$ et $P_2$, on commence par tracer la droite $\mathcal D$ entre $P_1$ et $P_2$. Cette droite possède une troisième intersection $P_3$ avec la courbe $E$. On note alors $P_1 + P_2$ la réfléction de $P_3$ par rapport à l'axe des abscisses.
\end{deff}
...
\begin{tikzpicture}
  \begin{axis}[
        ...
        \addplot [red] expression[domain=1:2]{sqrt(x*(x^2-1))}
                node[right] {$y^2=x(x-1)(x+1)$};
        \addplot [red] expression[domain=1:2]{-sqrt(x*(x^2-1))};
        \addplot [blue] expression[domain=-2:2]{(0.2+0.32*x)/0.73};
        \filldraw[black] (-0.93, -0.13) circle (2pt) node[anchor=north]{};
        \filldraw[black] (-0.2, 0.19) circle (2pt) node[anchor=north]{};
        \filldraw[black] (1.28, 0.83) circle (2pt) node[anchor=north]{};
        \filldraw[black] (1.28, -0.83) circle (2pt) node[anchor=north]{};
  \end{axis}
\end{tikzpicture}
\end{center}
...
Cette définition d'est pas claire pour plusieures raisons. Quelle sont les  droites passant par $\infty$ par exemple ? On souhaite que $\infty$ soit l'élément neutre soit le neutre de cette loi, donc si par exemple $P_1=\infty$, on définit : $P_1+P_2 = \infty+P_2 = P_2$.
Ensuite que ce passe-t'il si $P_1=P_2$ ? On considère pour $\mathcal D$ dans ce cas là la tangente à $E$ passant par $P_1$.
...
\item Si $x_1=x_2$ et $y_1\neq y_2$, la droite $\mathcal D$ est verticale et on définit $P_1+P_2$ comme valant $\infty$. 

\item Enfin si $P_1 = P_2$, on considère la tangente à $E$ passant par $P_1$. Pour trouver la pente de la tangente, on dérive l'équation de la courbe par rapport à $x$ : $2y\frac{dy}{dx} = 3x^2 + a$, donc $m = \frac{3x_1^2 + a}{2y_1}$. Si  $y_1 \neq  0$, $m$ est bien défini et on construit $P_1+P_1$ comme précédemment : la droite $\mathcal D$ possède une intersection de multiplicité double en $P_1$ puisqu'elle est tangente, et on obtient la troisième intersection en utilisant les relations racines-coefficients.

\item Dans le cas précédent, on peut montrer que si $y_1 = 0$, alors $3x_1^2 + a \neq 0$. 
