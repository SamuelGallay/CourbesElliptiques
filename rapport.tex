...
\begin{center}
  \begin{tikzcd}
    & & E\ar[dd, "\psi", dashed] \\
    E \ar[r, "\pi_p"] \ar[rru, "\pi_{p^2}", bend left=20] \ar[rrd, "{[p]}", bend right=20]  & E^{(p)} \ar[ru, "\pi_p"] \ar[rd, "\h{\pi_p}"] & \\
    & & E
  \end{tikzcd}
\end{center}
...
\begin{center}
  \begin{tikzcd}
    ...
    E_A
    \ar[dr, red,
    "\Ker(\phi_B')=\langle m_B\phi_A(P_B) + n_B\phi_A(Q_B)\rangle"{rotate=-19, anchor=south}, 
    "\phi_B'"{below, rotate=-19}]
    \ar[dddd, blue, shift right=3px]
    &[105pt] \\[20pt]

    &  
    &E_{BA} 
    \ar[dd, leftrightsquigarrow, "\simeq" {anchor=south, rotate=-90}] 
    \\[-18pt]

    E_0 
    \ar[uur, blue,
    "\Ker(\phi_A) = \langle m_AP_A +n_AQ_A \rangle" {anchor=south, rotate = 32}, 
    "{\phi_A,\ \phi_A(P_B),\ \phi_A(Q_B)}"{below, rotate=32}] 
    \ar[ddr, red,
    "\Ker(\phi_B) = \langle m_BP_B +n_BQ_B \rangle"{anchor=south, rotate = -32, below}, 
    "{\phi_B,\ \phi_B(P_A),\ \phi_B(Q_A)}"{above, rotate=-32}]  
    & & 
    \\[-18pt]
    ...
    &  E_B
    \ar[ur, blue,
    "\Ker(\phi_A')=\langle m_A\phi_B(P_A) + n_A\phi_B(Q_A)\rangle"{rotate = 19, below}, 
    "\phi_A'"{above, rotate=19}] 
    \ar[uuuu, red, shift right=3px]
    &
  \end{tikzcd}
\end{center}
...
La proposition \ref{nombre_cycliques} assure qu'il y a suffisament de sous groupes cycliques dans les $2^a$ et $3^b$-torsions de $E_0$ pour qu'il y ait suffisament de courbes $E_A$ et $E_B$ différentes possibles. La proposition \ref{recherche_base} permet en tirant des éléments au hasard de trouver rapidement une base des torsions.
